...
Цель: GDD §7 (цикл дня/ночи). Задел уже есть: флаг `dayOver` в world-store ставится при исчерпании AP, UI гейтится по нему (действия карт/локации блокируются с подсказкой «The day is over»), `world-store.newDay()` и `card-store.reshuffleForNewDay()` готовы (сброс AP/часов, expired-TTL hand-size, reshuffle + новая раздача).

- [ ] Исчерпание AP → окно «Another day/Another night»: `Move`, `Do it | 3` (отсрочка на реальные 30 мин, один раз в день), `Deep` (заглушка до ответа по GDD §12)
...
## Решения по открытым вопросам (GDD §12)
